...
class MenuDrawer extends HTMLElement {
  ...
  openMenuDrawer(summaryElement = false) {
    ...
    if(this.classes.open == 'mini-cart--open') {
      document.dispatchEvent(new CustomEvent('mini-cart:open', {detail: {}}))
    }
  }
...
class HeaderDrawer extends MenuDrawer {
  ...
  connectedCallback() {
    const searchToggle = this.querySelector('.modal__toggle-open .field')
    searchToggle.addEventListener('click', this.toggleSearch.bind(this))
  }

  toggleSearch(e) {
    this.closeMenuDrawer(e)
    const event = new CustomEvent('search:trigger', { detail: {} })
    this.dispatchEvent(new CustomEvent('drawer:force-close'))
...
class SliderComponent extends HTMLElement {
  ...
  onButtonClick(event) {
    ...
    if (this.enableSliderLooping) {
      ...
      if (direction === 1 && this.slider.scrollLeft >= maxScrollLeft) {
        // If next button clicked on last slide
        this.slider.scrollLeft = 0;
        return;
      } else if (direction === -1 && this.slider.scrollLeft <= 0) {
        // If previous button clicked on first slide
        this.slider.scrollLeft = maxScrollLeft;
        return;
      }
    }

    this.slideScrollPosition = this.slider.scrollLeft + (direction * step * this.sliderItemOffset);
  ...
  checkButtonVisibility() {
    const totalSlidesWidth = Array.from(this.sliderItems).reduce((total, item) => total + item.clientWidth, 0);
    const sliderWidth = this.slider.clientWidth;

    if (totalSlidesWidth <= sliderWidth) {
      this.prevButton.setAttribute('disabled', 'disabled');
      this.nextButton.setAttribute('disabled', 'disabled');
    } else {
      this.prevButton.removeAttribute('disabled');
...
class SliderComponentSwiper extends HTMLElement {
  constructor() {
    super()

    this.sliderEl = this.querySelector('.slider-wrapper');
    this.prevButton = this.querySelector('button[name="previous"]');
    this.nextButton = this.querySelector('button[name="next"]');

    this.slidesPerMobile = this.dataset.slidesPerMobile || 1
  }
...
customElements.define('slider-component-swiper', SliderComponentSwiper);
...
class VariantSelects extends HTMLElement {
  ...
  updateGangMedia() {
    ...
    if (this.currentGangValue !== gangValue) {
      ...
      Array.from(newMedias).reverse().forEach(function(newMedia, idx) {
        if(!newMedia.classList.contains('images-with-text__image')) {
          newMedia.classList.add('gang__active');
        } else {
          if(idx < 5) newMedia.classList.add('gang__active');
        }
  ...
  CJKDetect(someString) {
    // Korean
    if (someString.search("[\uac00-\ud7a3]") >= 0) return "ko";
    // Japanese
    if (someString.search("[\u3040-\u30ff]") >= 0) return "ja";
    // Chinese
    if (someString.search("[\u4e00-\u9FFF]") >= 0) return "zh";
  
    return "none";
  }

  getSlug(someString) {
    if (this.CJKDetect(someString) != "none") return someString;
  ...
  updateMedia() {
    ...
    parent.prepend(newMedia);
    parent.classList.add('product__media-no-animate');
    if (parent.parentElement.hasOwnProperty('goToFirstSlide') && typeof parent.parentElement.goToFirstSlide === 'function') {
      parent.parentElement.goToFirstSlide();
    }
  ...
  renderProductInfo() {
    ...
    fetch(`${this.dataset.url}?variant=${this.currentVariant.id}&section_id=${this.dataset.section}`)
      ...
      .then((responseText) => {
        ...
        if (this.currentVariant) this.toggleAddButton(!this.currentVariant.available, theme.variantStrings.soldOut);
  ...
  updateCustomLiquid(responseText) {
    const html = new DOMParser().parseFromString(responseText, 'text/html');
    const destinations = document.querySelectorAll(`.custom__liquid-${this.dataset.section}`);
    const sources = html.querySelectorAll(`.custom__liquid-${this.dataset.section}`);
  ...
  setUnavailable() {
    ...
    productForms.forEach((productForm) => {
      const addButton = productForm.querySelector('[name="add"]');
      if (!addButton) return;
      addButton.textContent = theme.variantStrings.unavailable;
...
class VariantRadios extends VariantSelects {
  ...
  enableVariantOption(group, obj) {
    ...
    input.removeAttribute('data-crossout');

    // Variant sold out - cross out option (remains selectable)
    if (obj.soldOut) {
      if (this.hasAttribute('data-swatch-cross')) {
        if (this.hasAttribute('data-swatch-oos-click')) {
          input.setAttribute('data-crossout', '');
        } else {
...
class CountdownTimer extends HTMLElement {
  constructor() {
    ...
    this.date = new Date(dateArray[2], dateArray[1] - 1, dateArray[0]).getTime();
...
class CartQuickAdd extends HTMLElement {
  constructor() {
    super();

    this.wrapper = this.querySelector('.QuickCart__wrapper')
    this.miniCart = document.querySelector('mini-cart');
    this.addToCartButton = this.querySelector('[data-add-to-cart]')
    this.variants = this.querySelectorAll('.ProductCard__variant [data-variant-id]')
    this.quickCartTriggerMobile = this.querySelector('.QuickAdd__trigger--mobile .button')
    
    this.addToCartButton.addEventListener('click', this.addToCart.bind(this))
    this.quickCartTriggerMobile.addEventListener('click', this.showQuickCart.bind(this))
    
    this.variants.forEach(item => {
      item.addEventListener('click', this.updateAddToCart.bind(this))
    })
  }

  showQuickCart(event) {
    event.preventDefault()
    const currentActive = document.querySelectorAll('.quick-cart-active')
    currentActive.forEach(item => item.classList.remove('quick-cart-active'))
    this.wrapper.classList.add('quick-cart-active')
  }

  updateAddToCart(event) {
    event.preventDefault()
    const target = event.currentTarget

    const available = target.dataset.available == "true" ? true : false

    const id = target.dataset.variantId

    if(!available || !id) return;

    const currentSelected = this.querySelector('.ProductCard__variant .selected')
    if(currentSelected) currentSelected.classList.remove('selected')
    target.classList.add('selected')

    this.addToCartButton.setAttribute('data-value', id)
    this.addToCartButton.classList.add('active')
  ...
  addToCart(event) {
    ...
    .then(parsedState => {
      if(parsedState.status === 422) {
        ...
        this.addToCartButton.classList.remove('active')
        this.wrapper.classList.remove('quick-cart-active')
      }
    })
    .catch(e => {
      console.log(e)
...
customElements.define('cart-quick-add', CartQuickAdd);



class CartRecommendationSelectTrigger extends HTMLElement {
  constructor() {
    super();

    this.scriptTag = this.querySelector('script[type="application/json"]');
  ...
  handleClick(event) {
    ...
    const jsonData = JSON.parse(this.scriptTag.textContent);
    console.log(jsonData)
    document.dispatchEvent(new CustomEvent('product-data:load', { 
      detail: jsonData 
    }));
  }
}

customElements.define('cart-recommendation-select-trigger', CartRecommendationSelectTrigger);
